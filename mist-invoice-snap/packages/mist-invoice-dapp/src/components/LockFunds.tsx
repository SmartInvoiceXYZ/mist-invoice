...
import { uploadDisputeDetails } from '../utils';
import { Loader } from './Loader';

export type LockFundsProps = {
  invoice: any;
  balance: bigint;
  tokenData: any;
};

export const LockFunds: React.FC<LockFundsProps> = ({
  invoice,
  balance,
  tokenData,
}) => {
  ...
  useEffect(() => {
    if (chainId && tokenData) {
      setTokenInfo(getTokenInfo(chainId, token, tokenData));
    }
  }, [chainId, tokenData]);

  const fee = `${formatUnits(balance / resolutionRate, tokenInfo.decimals)} ${
    tokenInfo.symbol
  }`;
  ...
  const lockFunds = useCallback(async () => {
    if (provider && !locking && balance > 0 && disputeReason) {
      try {
        setLocking(true);
    ...
    return (
      <VStack w="100%" spacing="1rem">
        ...
        >
          ...
          >
            <Image width="2rem" src="/assets/lock.svg" />
          </Flex>
        </Flex>
      </VStack>
    );
  ...
  return (
    <VStack w="100%" spacing="1rem">
      ...
      <Button
        ...
        w="100%"
      >
        {`Lock ${formatUnits(balance, tokenInfo.decimals)} ${tokenInfo.symbol}`}
      </Button>
      {chainId && isKnownResolver(chainId, resolver) && (
        <Link
