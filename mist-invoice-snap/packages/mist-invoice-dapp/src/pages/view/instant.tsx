...
import { BigNumberish, isAddress, Provider } from 'ethers';
...
import { InvoiceNotFound } from '../../components/InvoiceNotFound';
...
export type ViewInstantInvoiceProps = {
  match: {
    params: {
      hexChainId: string;
      invoiceId: string;
...
export const ViewInstantInvoice: React.FC<ViewInstantInvoiceProps> = ({
  match: {
  ...
  const [lateFeeAmount, setLateFeeAmount] = useState<BigNumberish>();
  const [lateFeeTimeInterval, setLateFeeTimeInterval] =
  ...
  useEffect(() => {
    if (invoice && totalDue !== BigInt(0) && deadline) {
      setLateFeeTotal(totalDue - invoice.total.toBig());
    }
  }, [invoice, deadline, totalDue]);

  const getValues = async (provider: Provider, invoice: InvoiceResult) => {
    // Get Balance
    try {
      setBalanceLoading(true);
    ...
    try {
      ...
      setTotalDue(invoice.total.toBig());
    }

    // Get Deadline, Late Fee and its time interval
    try {
      const d = await getDeadline(provider, invoice.address);
      setDeadline(d as BigNumberish);
      const { amount, timeInterval } = await getLateFee(
        provider,
        invoice.address,
      );
      setLateFeeAmount(amount as BigNumberish);
      setLateFeeTimeInterval(timeInterval as BigNumberish);
    } catch (lateFeeError) {
  ...
  const deposited = BigInt(released).add(balance);
  const due =
