...
contract MistInvoiceEscrowWrapper is Verifier {
    ...
    uint256 public constant CLIENT_SIGNAL =
        uint256(keccak256("client")) % SNARK_SCALAR_FIELD;
    uint256 public constant PROVIDER_SIGNAL =
        uint256(keccak256("provider")) % SNARK_SCALAR_FIELD;
    ...
    function resolve(
        ...
        uint256 forClient = (_clientAward / (_clientAward + _providerAward)) *
            (postBalance - preBalance);
        uint256 forProvider = (_providerAward /
            (_clientAward + _providerAward)) * (postBalance - preBalance);
